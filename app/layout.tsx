...
function getThemeFromPreset(preset: string): ThemePalette {
...
function isVeryLight(hex: string) {
  ...
  // Perceived luminance threshold to avoid white-like text on light backgrounds.
...
async function BodyWithTheme({
  ...
  const presetTheme = getThemeFromPreset(settings.theme_preset);
  ...
  return (
    <body
      style={themeVars}
      className={`${interClass} ${displayClass} antialiased`}
    >
      {children}
    </body>
  );
